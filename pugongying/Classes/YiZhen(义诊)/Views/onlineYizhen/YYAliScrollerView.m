...
@interface YYAliScrollerView ()<UITextViewDelegate>
...
@implementation YYAliScrollerView
...
- (void)addLabelTextField{
    ...
    self.storeAddressTextField = [self addLabelAndTextFieldWithLabelFrame:CGRectMake(labelX, labelY, labelW, labelH) andTextFieldFrame:CGRectMake(textfieldX, labelY, textfieldW, labelH) andlabelText:@"公司全称/账号ID:" placeholder:nil withTag:0];
...
//    self.fouthBtnsArray = [self addBtnToViewWithLabelFrame:CGRectMake(labelX, firstBtnY + (labelH + marginH)*3, labelW, labelH) btnsTitle:@[@"常参加", @"有时候", @"不参加"] labelTitle:@"是否参加活动:"];
    
    self.fiveBtnsArray = [self addBtnToViewWithLabelFrame:CGRectMake(labelX, firstBtnY + (labelH + marginH)*1, labelW, labelH) btnsTitle:@[@"全职做", @"偶尔做"] labelTitle:@"店铺操作时间:"];
...
- (NSMutableArray *)addBtnToViewWithLabelFrame:(CGRect)labelFrame btnsTitle:(NSArray *)titleArrays labelTitle:(NSString *)labelTitle{
    ...
    else if ([labelTitle isEqualToString:@"是否参加活动:"]){
        index = 30;
    }
...
- (CGFloat)getTextViewHeight{
    NSString *deviceString = [YYAliScrollerView deviceVersion];
