...
@interface YYInternationalScrollerView ()<UITextViewDelegate>

@property (nonatomic, weak) UIButton *coverBtn;

@end
static NSString *placeholderStr = @"小pu温馨提示：请在此输入您对店铺的想法或者总结哦！\n\n如：每月消耗费用数、橱窗位数量、询盘量等）\n您资料提供的越详细，专家诊断的越精湛噢！！";

@implementation YYInternationalScrollerView
...
- (instancetype) initWithFrame:(CGRect)frame{
    if (self = [super initWithFrame:frame]) {
        ...
        [self addBtnsToScrollerViewWithFirstBtnY:CGRectGetMaxY(self.storeAddressTextField.frame) + YY10HeightMargin];
...
- (void)addLabelTextField{
    ...
    self.storeAddressTextField = [self addLabelAndTextFieldWithLabelFrame:CGRectMake(labelX, labelY, labelW, labelH) andTextFieldFrame:CGRectMake(textfieldX, labelY, textfieldW, labelH) andlabelText:@"公司全称:" placeholder:@"Please Describe English" withTag:0];
...
//    self.thirdBtnsArray = [self addBtnToViewWithLabelFrame:CGRectMake(labelX, firstBtnY + (labelH + marginH)*2, labelW, labelH) btnsTitle:@[@"直通车", @"做钻展", @"没有做"] labelTitle:@"是否付费推广:"];
    
    self.fouthBtnsArray = [self addBtnToViewWithLabelFrame:CGRectMake(labelX, firstBtnY, labelW, labelH) btnsTitle:@[@"全职做", @"偶尔做"] labelTitle:@"店铺操作时间:"];
    
    self.fiveBtnsArray = [self addBtnToViewWithLabelFrame:CGRectMake(labelX, firstBtnY + (labelH + marginH)*1, 145, labelH) btnsTitle:@[@"50以下", @"50以上"] labelTitle:@"每周上传产品数量:"];
    
    self.sixBtnsArray = [self addBtnToViewWithLabelFrame:CGRectMake(labelX, firstBtnY + (labelH + marginH)*2, 140, labelH) btnsTitle:@[@"是", @"否"] labelTitle:@"是否使用P4P业务:"];
    
    //增加下面的补充情况输入框
    CGFloat textViewLabelY = firstBtnY + (labelH + marginH) * 3;
...
- (NSMutableArray *)addBtnToViewWithLabelFrame:(CGRect)labelFrame btnsTitle:(NSArray *)titleArrays labelTitle:(NSString *)labelTitle{
    ...
    else if ([labelTitle isEqualToString:@"每周上传产品数量:"]){
        index = 40;
    }
    else if ([labelTitle isEqualToString:@"是否使用P4P业务:"]){
        index = 50;
    }
    ...
    for (int i = 0; i < titleArrays.count; i++) {
        ...
        [btn addTarget:self action:@selector(internationalBtnsClickWithBtn:) forControlEvents:UIControlEventTouchUpInside];
    }
 ...
 * 店铺操作时间数组tag40,41,42
 */
- (void)internationalBtnsClickWithBtn:(UIButton *)btn{
//    if (btn.tag/10 == 0) {
...
//    }
    ...
    else if (btn.tag/10 == 5){
         //        YYLog(@"是否适用P4P业务");
        for (UIButton *subBtn in self.sixBtnsArray) {
            subBtn.selected = NO;
        }
    }
    btn.selected = YES;
...
- (CGFloat)getTextViewHeight{
    NSString *deviceString = [YYInternationalScrollerView deviceVersion];
